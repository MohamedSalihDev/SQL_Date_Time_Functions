...
SELECT 
	timeofday(); -- returns text as string value
...
SELECT
	emp_no
	,AGE(to_date, from_date) AS manager_time
FROM 
	dept_manager;
...
SELECT 
	emp_no
	, AGE(CURRENT_DATE, from_date) AS Age
FROM 
	dept_manager
WHERE 
	emp_no IN ('110039', '110114', '110228', '110420', '110567', '110854', '111133', '111534','111939');
...
SELECT
	CASE
		WHEN AGE(to_date, from_date) < AGE(CURRENT_DATE, from_date) 
			THEN AGE(to_date, from_date)
...
SELECT
	...
	, s.from_date
	, s.to_date
	,AGE(s.to_date, s.from_date) AS Salary_Duration
FROM
	employees e
JOIN
	salaries s
...
SELECT
	...
	,last_name
	,birth_date
	,EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM birth_date) AS employee_age
FROM
	employees;
...
FROM
	employees
WHERE
EXTRACT(YEAR FROM hire_date) - EXTRACT(YEAR FROM birth_date) <= 25;
...
SELECT
	...
	,sub_category
	,sales
	,EXTRACT( MONTH FROM ship_date) AS sales_month
FROM
	sales
WHERE
	sub_category = 'Chairs' AND order_id LIKE '%2015%';
...
SELECT
	order_date
	,TO_CHAR(order_date,'Month FMDDth, YYYY')
FROM
	sales;
...
SELECT 
	order_date
	, TO_CHAR(order_date, 'Month DAY, YYYY')
FROM 
	sales;
...
SELECT 
	*
	, TO_CHAR(hire_date, 'Day, Month DD YYYY') AS hired_date
FROM 
	employees;
...
-- for each month for chair sub category in the year 2015
SELECT
	TO_CHAR(ship_date, 'Month') AS full_sales_month
	,SUM(sales) AS monthly_total
FROM
	sales
WHERE
...
SELECT
	*
	,TO_DATE(hire_date, 'YYY-MM-DD') AS hire_date
FROM
	employees;
